...
pub async fn share_promote(
    ...
    let _ = app.emit("tasks-changed", &doc.to_json());
...
#[tauri::command]
pub async fn share_accept(app: AppHandle, url: String) -> Result<String, String> {
    let manager = current_manager()
        .map_err(|e| e.to_string())?
...
#[tauri::command]
pub async fn share_leave(app: AppHandle, project_id: String) -> Result<(), String> {
    let manager = current_manager()
        .map_err(|e| e.to_string())?
