...
const { useState: aUseState, useEffect: aUseEffect, useMemo: aUseMemo, useRef: aUseRef, useCallback: aUseCb } = React;
...
function App() {
  // theme
  const [theme, setTheme] = aUseState(window.__TWEAKS?.theme || "tokyonight");
  aUseEffect(() => {
  ...
  const commands = aUseMemo(() => {
    ...
    return [
      ...
      ...projectCommands,
      window.makeSyncCommand(sync.account, sync.openSync),
      ...
      { id: "theme-tokyonight", title: "theme: tokyo night", run: () => setTheme("tokyonight") },
      { id: "theme-catppuccin", title: "theme: catppuccin",  run: () => setTheme("catppuccin") },
      { id: "theme-gruvbox",    title: "theme: gruvbox",     run: () => setTheme("gruvbox") },
      { id: "theme-ubuntu",     title: "theme: ubuntu",      run: () => setTheme("ubuntu") },
  ...
  aUseEffect(() => {
    const handler = (e) => {
      ...
      switch (e.key) {
        case "j": case "ArrowDown": e.preventDefault(); setCursor(c => Math.min(viewTasks.length - 1, c + 1)); break;
        case "k": case "ArrowUp":   e.preventDefault(); setCursor(c => Math.max(0, c - 1)); break;
        case "J": e.preventDefault(); setCursor(c => Math.min(viewTasks.length - 1, c + 5)); break;
        case "K": e.preventDefault(); setCursor(c => Math.max(0, c - 5)); break;
        ...
        case "o": e.preventDefault(); openQuickAdd(); break;
        case "O": e.preventDefault(); openQuickAdd(); break;
        case "a": e.preventDefault(); openQuickAdd(); break;
        case "x":
        case " ":
        case "Enter":
          e.preventDefault(); if (currentTask) toggleDone(currentTask.id); break;
  ...
  // edit mode messaging for host tweaks
  aUseEffect(() => {
    const listener = (e) => {
      const msg = e.data || {};
      if (msg.type === "__activate_edit_mode")   window.__tweaksShow && window.__tweaksShow();
      if (msg.type === "__deactivate_edit_mode") window.__tweaksHide && window.__tweaksHide();
    };
    ...
    <div style={S.root}>
      ...
      {sync.dialog && (
        <window.SyncDialog
          ...
          setAccount={sync.setAccount}
          onClose={() => sync.setDialog(null)}
          flash={sync.flash}
          variant="modal"
        />
      )}
      ...
      {window.Tweaks ? <window.Tweaks setTheme={setTheme} theme={theme} /> : null}
    </div>
  );
}
  ...
  return (
    <div style={S.detailInner}>
      ...
      <div style={S.metaGrid}>
        ...
        <div style={S.metaVal}>
          <select value={task.list} onChange={e => onMove(e.target.value)} style={S.select}>
        ...
        <div style={S.metaVal}>
          <select value={task.ctx || ""} onChange={e => onUpdate({ ctx: e.target.value })} style={S.select}>
            <option value="">—</option>
        ...
        <div style={S.metaVal}>
          <select value={task.due || ""} onChange={e => onUpdate({ due: e.target.value })} style={S.select}>
            <option value="">—</option>
  ...
  return (
    <div style={S.statusBar}>
      ...
      <span style={S.statusSeg}>
        <window.SyncDot account={syncAccount} onClick={onSyncClick} labelled />
      </span>
...
const S = {
  ...
  rowGutter: { display: "flex", alignItems: "center", justifyContent: "center", height: 20 },
  check: {
...
window.DeferPicker = DeferPicker;
