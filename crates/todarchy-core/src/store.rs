...
pub async fn load(sink: &dyn EventSink) -> Result<Value> {
    ...
    if let Some((base, doc_id)) = crate::config::server_config()? {
        match pull_from_server(&base, &doc_id, &mut doc).await {
            Ok(()) => crate::sync::record_success(sink),
            Err(e) => crate::sync::record_error(sink, format!("server pull: {e}")),
        }
    }
    ...
    if let Some(sync_path) = crate::config::sync_doc_path()? {
        ...
        match step_error {
            Some(reason) => crate::sync::record_error(sink, reason),
            None => crate::sync::record_success(sink),
        }
    } else {
...
/// if configured.
pub async fn save(sink: &dyn EventSink, data: Value) -> Result<()> {
    let local_path = automerge_path()?;
    let mut doc = TaskDoc::load(&local_path)?;
...
pub async fn delete_many(sink: &dyn EventSink, root_key: &str, ids: &[String]) -> Result<()> {
    let local_path = automerge_path()?;
    let mut doc = TaskDoc::load(&local_path)?;
