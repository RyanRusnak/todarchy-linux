...
pub async fn run_loop(sink: Arc<dyn EventSink>) {
    ...
    loop {
        ...
        if let Some(rx) = rx_opt.as_mut() {
            ...
            if saw_write {
                if let Err(e) = apply_sync_update(&*sink).await {
...
pub async fn server_poll_loop(sink: Arc<dyn EventSink>) {
    ...
    loop {
        ...
        match crate::store::load(&*sink).await {
            Ok(json) => {
...
async fn apply_sync_update(sink: &dyn EventSink) -> Result<()> {
    ...
    match result {
        ...
        Err(e) => crate::sync::record_error(sink, e.to_string()),
    }
    Ok(())
}
